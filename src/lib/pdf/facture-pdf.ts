// ── Génération du PDF facture — partagé client (téléchargement) + serveur (email) ──
import type { Prisma } from "@prisma/client";
import { getCountry, type Country } from "@/lib/countries";
import { loadJsPDF } from "./load-js-pdf";

export type FactureWithLines = Prisma.FactureGetPayload<{ include: { lines: true } }>;

export interface FacturePdfCompany {
  logo?: string | null;
  name?: string | null;
  siret?: string | null;
  address?: string | null;
...
/** Construit le document PDF de la facture (sans déclencher le téléchargement) */
export async function buildFacturePdfDoc(
  ...
  if (company.logo) {
    try {
      doc.addImage(company.logo, "PNG", margin + 2, 4, 24, 24);
      doc.setTextColor(226, 176, 74);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(16);
      doc.text(company.name || "FACTURE", margin + 30, 16);
      doc.setFontSize(8);
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(18);
      doc.text(company.name || "FACTURE", margin, 18);
      doc.setFontSize(8);
      doc.setTextColor(255, 255, 255);
      doc.setFont("helvetica", "normal");
  ...
  if (company.name || company.siret || company.address || company.email) {
    doc.setFontSize(7.5);
    doc.setTextColor(100, 100, 100);
    doc.setFont("helvetica", "normal");
    let infoY = y;
    if (company.name) { doc.text(company.name, pageWidth - margin, infoY, { align: "right" } as any); infoY += 4; }
    if (company.siret) { doc.text(`N° TVA : ${company.siret}`, pageWidth - margin, infoY, { align: "right" } as any); infoY += 4; }
    if (company.address) { doc.text(company.address, pageWidth - margin, infoY, { align: "right" } as any); infoY += 4; }
    if (company.email) { doc.text(company.email, pageWidth - margin, infoY, { align: "right" } as any); }
    y = infoY + 8;
  }
  ...
  const displayIban = facture.iban || company.iban || "";
  const displayBic = facture.bic || company.bic || "";
  ...
  if (company.customLegalMentions) {
    y += 4;
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(100, 100, 100);
    const customLines = company.customLegalMentions.split("\n").filter(Boolean);
...
/** Nom de fichier standard pour le PDF facture */
export function facturePdfFilename(facture: { number: string }): string {
  return `facture-${facture.number.toLowerCase()}.pdf`;
}
