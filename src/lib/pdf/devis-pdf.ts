// ── Génération du PDF devis — partagé client (téléchargement) + serveur (email) ──
import type { Prisma } from "@prisma/client";
import { getCountry, type Country } from "@/lib/countries";
import { loadJsPDF } from "./load-js-pdf";

export type DevisWithLines = Prisma.DevisGetPayload<{ include: { lines: true } }>;

export interface DevisPdfCompany {
  logo?: string | null;
  name?: string | null;
...
export async function buildDevisPdfDoc(
  ...
  const { signatureData, company = {} } = opts;
  ...
  if (company.logo) {
    try {
      ...
      doc.setFontSize(16);
      doc.text(company.name || "MONDEVIS", margin + 30, 16);
      doc.setFontSize(8);
  ...
  doc.text(`Date : ${new Date(devis.createdAt).toLocaleDateString("fr-FR")}`, margin, y);
  if (devis.profession) {
    y += 5;
...
/** Nom de fichier standard pour le PDF devis */
export function devisPdfFilename(devis: { number: string }): string {
  return `devis-${devis.number.toLowerCase()}.pdf`;
}
